...
| immediate | Whether the first iteration runs without waiting | true or false | false | When true, first iteration starts immediately; when false, waits for phase delay | Ignored for manual Loops (interval=0) | Use true when the objective is urgent; false for scheduled alignment |
