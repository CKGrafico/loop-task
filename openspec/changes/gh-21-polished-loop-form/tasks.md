> Generated retroactively: implementation proceeded directly from `proposal.md` + `design.md` across several WIP commits before a task board existed. Tasks below are marked against verified current code (`git log`, direct inspection) so this change can pass through `tasks` and be archived cleanly. Only genuinely remaining work is left unchecked.
...
## 2. Per-field validation display
