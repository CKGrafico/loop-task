...
- [x] 1.1 Add `HTTP_API_PORT` (8845) and `HTTP_API_HOST` (`127.0.0.1`) to `src/config/constants.ts` <!-- agent: basic-engineer.build, depends_on: none, touches: src/config/constants.ts -->
...
- [x] 3.1 Add `GET /api/docs` (Swagger UI HTML served inline) and `GET /api/openapi.json` (OpenAPI 3.0 spec) to `HttpApiServer` <!-- agent: basic-engineer.build, depends_on: 2.6, touches: src/daemon/http-server.ts -->

## 4. Daemon Integration

- [x] 4.1 Instantiate `HttpApiServer` in `src/daemon/index.ts`, start it alongside `IpcServer`, wire `broadcastEvent` into the push path, handle `EADDRINUSE` gracefully, close on shutdown <!-- agent: basic-engineer.build, depends_on: 2.2,2.3,2.4,2.5,2.6,3.1, touches: src/daemon/index.ts -->
...
- [x] 6.1 Write unit tests for route matcher and JSON response envelope helpers in `tests/http-server.test.ts` <!-- agent: basic-engineer.build, depends_on: 2.1, touches: tests/http-server.test.ts -->
- [x] 6.2 Write integration tests for loop, task, project, log, SSE, and Swagger endpoints in `tests/http-api.test.ts` <!-- agent: basic-engineer.build, depends_on: 4.1, touches: tests/http-api.test.ts -->
