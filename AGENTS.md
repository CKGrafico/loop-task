...
2. **Send keystrokes** to exercise the board: arrows / `j` `k` to move, `Enter` to edit/open logs, `n` to create, `/` to search, `esc` to quit. Modifier combos go through as the real terminal delivers them — this is also how you reproduce terminal-specific key issues.
